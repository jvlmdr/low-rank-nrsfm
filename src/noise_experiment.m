  ...
  make_solver(...
    @(projections, rotations) nrsfm_find_structure_adaptor(...
      @(projections, rotations) find_structure_constrained_nuclear_norm(...
        projections, rotations, 1, 400, 10, 10, 10), ...
...
%  make_solver(...
%    @(projections, rotations, structure, basis, coeff) ...
%      nrsfm_balm_approximate(projections, rotations, coeff), ...
%    'BALM', 'balm-approximate'), ...
%  make_solver(...
%    @(projections, rotations, structure, basis, coeff) ...
%      nrsfm_balm_metric_projections(projections, rotations, coeff), ...
%    'BALM with metric projections', 'balm-metric'), ...
%  make_solver(...
%    @(projections, rotations, structure, basis, coeff) ...
%      nrsfm_metric_projections(projections, rotations, coeff), ...
%    'Alternation with metric projections', 'alternation-metric'), ...
%  make_solver(...
%    @(projections, rotations, structure, basis, coeff) nrsfm_fixed_rank(...
%      projections, rotations, K, 1, 1, 400, 10, 10, 10), ...
%    'Fixed rank', 'fixed-rank'), ...
%  make_solver(...
%    @(projections, rotations, structure, basis, coeff) ...
%      nrsfm_constrained_nuclear_norm(projections, rotations, 1, 1, 400, 10, ...
%        10, 10), ...
%    'Nuclear norm (constrained)', 'nuclear-constrained'), ...
%  arrayfun(@(lambda) make_solver(...
%      @(projections, rotations, structure, basis, coeff) ...
%        nrsfm_nuclear_norm_regularizer(projections, rotations, lambda, 1, ...
%          400, 10, 10, 10), ...
